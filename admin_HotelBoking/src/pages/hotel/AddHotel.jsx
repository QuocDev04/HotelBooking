...
const AddHotel = () => {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: async (data) => {
            try {
                return await instance.post("/hotel",data);
            } catch (error) {
        ...
        onSuccess: () => {
            messageApi.open({
                type: "success",
                content: "Bạn thêm khách sạn thành công",
            });
            form.resetFields();
        },
        onError: () => {
            messageApi.open({
                type: "error",
                content: "Bạn thêm khách sạn thất bại. Vui lòng thử lại sau!",
            });
        },
    });    const locationData = [
        {
    ...
    return (
        <>
            <div className="flex items-center justify-between mb-8">
                <h1 className="text-3xl font-semibold text-blue-600">Thêm Khách Sạn</h1>
            </div>
            {contextHolder}
            <div className="bg-white p-8 rounded-lg shadow-lg">
                <Form form={form} name="add-room" layout="vertical" validateTrigger="onBlur" onFinish={onFinish}>
                    <Row gutter={[24, 24]}>
                        {/* Bên trái - 60% */}
